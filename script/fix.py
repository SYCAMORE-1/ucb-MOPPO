...
def evaluate_policy_PF(_dict_ , i, agent,w, env, times):
    ...
    if not os.path.exists(f'./model/{env_name}/dwc1_model'):
        # Create the directory (and any intermediate directories) if it doesn't exist
        os.makedirs(f'./model/{env_name}/dwc1_model')
    torch.save(agent.actor_critic.actor.state_dict(), f'./model/{env_name}/dwc1_model/{agent.id}_dwc_run{wandb.config.seed}_actor_model.pt')  
    torch.save(agent.actor_critic.critic.state_dict(), f'./model/{env_name}/dwc1_model/{agent.id}_dwc_run{wandb.config.seed}_critic_model.pt') 
    return _dict_
...
def main(config, env_name, env, recording_env, all_weights, eval_weights, weight_dim, debug=False):
    ...
    for i in tqdm(range(int(max_training_iter) // int(memory_size))):
        ...
        if i % 10 == 0:
            ...
            with open(f'./results/{env_name}/dwc1/last_pf_{wandb.config.seed}_{agent.global_step}.txt', 'w') as file:
                    file.write(str(_dict_))
            hv = compute_hypervolume(list(_dict_.values()))
            # sp = compute_sparsity(list(_dict_.values()))
            # hvs = hv-sp
            with open(f'./results/{env_name}/dwc1/hv{wandb.config.seed}.txt', 'a') as file:
                file.write(f'{str(hv)},')
...
if __name__ == '__main__':
    ...
    if MODE  == 0:
        ...
        directory = f'./results/{env_name}/dwc1/'
        ...
        # with open(f'./results/{env_name}/dwc1/pf_{seed}.txt', 'w') as file:
