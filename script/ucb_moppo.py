...
class Actor_Net(nn.Module):
    def __init__(self, n_observations,n_weights, n_actions,  num_cells, continous_action, log_std_init=0.0):
        ...
        self.layer3 = layer_init(nn.Linear(num_cells*self.factor+n_weights, n_actions), std=0.01)
    ...
    def forward(self, x, w):
        ...
        activation3 = self.layer3(residual)
        return activation3
...
class Critic_Net(nn.Module):
    def __init__(self, n_observations, n_weights, num_cells):
        ...
        self.layer1 = layer_init(nn.Linear(n_observations+n_weights, num_cells))
        self.layer2 = layer_init(nn.Linear(num_cells, num_cells*self.factor))
        self.layer3 = layer_init(nn.Linear(num_cells*self.factor+n_weights, 1), std=1.0)
...
def evaluate_policy_PF(_dict_ , i, agent,w, env, times):
    ...
    if not os.path.exists(f'./model/{env_name}/dwc4_model'):
        # Create the directory (and any intermediate directories) if it doesn't exist
        os.makedirs(f'./model/{env_name}/dwc4_model')
    torch.save(agent.actor_critic.actor.state_dict(), f'./model/{env_name}/dwc4_model/{agent.id}_dwc_run{wandb.config.seed}_actor_model.pt')  
    torch.save(agent.actor_critic.critic.state_dict(), f'./model/{env_name}/dwc4_model/{agent.id}_dwc_run{wandb.config.seed}_critic_model.pt') 
    return _dict_
...
def compute_differences_v2(data):
    result = {}
    for key, list_of_dicts in data.items():
        # This will store the result of differences for all consecutive dictionary pairs in the list
        differences = []
        # Iterate over pairs of consecutive dictionaries
        for i in range(len(list_of_dicts) - 1):
            current_dict = list_of_dicts[i]
            next_dict = list_of_dicts[i + 1]
            # Compute the difference for each dictionary pair
            diff_dict = {}
            for subkey in current_dict:
                # Subtract element-wise between current and next dictionaries
                diff_dict[subkey] = [
                    next_val - current_val for current_val, next_val in zip(current_dict[subkey], next_dict[subkey])
                ]
...
def find_array_index(list_of_arrays, target_array):
    # Iterate over each array in the list
    for index, current_array in enumerate(list_of_arrays):
        if np.array_equal(current_array, target_array):
            return index
    # Return -1 if the target array is not found
    return -1

def stack_data(d, temp):
    # Iterate over each key and list of values in temp
    for key, values in temp.items():
        # Extend the list associated with the key in d with the values from temp
        d[key].extend(values)
    return d


def main(config, env_name, env, recording_env, all_weights, eval_weights, weight_dim, mode,debug=False):
    ...
    agent_list = []
    pg_training_data = defaultdict(list)
    data_differences = defaultdict(list)
    ...
    # =================init pg models ===============
    for p_num in range(policy_num):
        temp = []
        for _ in range(weight_dim):
            elastic_net_obj = ElasticNet()
            temp.append(elastic_net_obj)
        pg_models.append(temp)
    ...
    for i in tqdm(range(int(max_training_iter) // int(memory_size))):
        ...
        weights_to_eval_collection = np.concatenate(list(weights_to_eval.values()))
        ...
        if i % train_data_collect_point == 0:
            ...
            for agent,weights in zip(agent_list,list(weights_to_train.values())):
                temp_dict_ = {}
                for w in weights:
                    k = find_array_index(weights_to_eval_collection,w)
                    temp_dict_= evaluate_policy_PF(temp_dict_, k, agent, w, recording_env, times=10)
                # {policy_0:[{w:[1,2],w2:[5,1]},{{w:[1,2],w2:[5,1]}],
                # policy_1:[{w:[1,2],w2:[5,1]},{{w:[1,2],w2:[5,1]}}
                pg_training_data[agent.id].append(temp_dict_)
                _dict_.update(temp_dict_)

            with open(f'./results/{env_name}/dwc4/last_pf_{wandb.config.seed}_{agent.global_step}.txt', 'w') as file:
                    file.write(str(_dict_))

            if weight_dim ==2:
                hv = compute_hypervolume(find_pareto_front(list(_dict_.values())))
            elif weight_dim ==3 :
                hv = compute_hypervolume(find_pareto_front_3d(list(_dict_.values())))
            # sp = compute_sparsity(list(_dict_.values()))
            # hvs = hv-sp
            with open(f'./results/{env_name}/dwc4/hv{wandb.config.seed}.txt', 'a') as file:
                file.write(f'{str(hv)},')
        ...
        if i >warm_up_iters and i >_F+pg_mdoel_train_point:
            _F = i
            if T == 0:
                T = i

            # Compute differences
                # {policy_0:[{w:[d1,d2],w2:[d5,d1]},{{w:[d1,d2],w2:[d5,d1]}],
                # policy_1:[{w:[d1,d2],w2:[d5,d1]},{{w:[d1,d2],w2:[d5,d1]}}
            temp_data_differences = compute_differences_v2(pg_training_data)
            data_differences= stack_data(data_differences,temp_data_differences)
            pg_training_data.clear()
            # Print the resulting differences
            # print(list(differences.values()))  # Convert to regular dict for printing if necessary
            # append_dicts_to_csv(list(data_differences.values()), f'./results/{env_name}/dwc5/train_data.csv')


            # _data = read_csv_to_dicts('train_data.csv')

            for policy_i in range(policy_num):
                trained_pg_models= []

                for _i in range(len(pg_models[policy_i])):
                    sub_training_data = []
                    for record in data_differences[policy_i]:
                        for w_idx,obj_delta in record.items():
                            w = weights_to_eval_collection[w_idx]
                            sub_training_data.append([w,obj_delta[_i]])
                ...
                # ==============train pg models with grid search parameter searching==============
                    ...
                    bagging_regressor = BaggingRegressor(pg_models[policy_i][_i])
                    ...
                    grid_search = GridSearchCV(estimator=bagging_regressor,param_grid=param_grid, cv=5, scoring='neg_median_absolute_error')
                    print(f'training pg model for obj {_i}')
                    grid_search.fit(x, y_norm)

                    
                    print(f"Best parameters for pg model{_i} for policy {policy_i}: {grid_search.best_params_}")
                ...
                for pg_idx, trained_pg_model in enumerate(trained_pg_models):
                    if not os.path.exists(f'./model/{env_name}/dwc4_model'):
                        # Create the directory (and any intermediate directories) if it doesn't exist
                        os.makedirs(f'./model/{env_name}/dwc4_model')
                    save_pg_model(trained_pg_model,f'./model/{env_name}/dwc4_model/policy_{policy_i}_pg_model_obj{pg_idx}.joblib')


        

                # ==============use prediction model to guide preference choice==============
                policy_to_eval = agent_list[policy_i]
                unsorted_hv = []
                last_pf_in_region = {}

                for ii, weights in enumerate(weights_to_eval[policy_i]):
                    last_pf_in_region= evaluate_policy_PF(last_pf_in_region, ii, policy_to_eval, w, recording_env, times=1)

                for ii, weights in enumerate(weights_to_eval[policy_i]):
                    temp_last_pf_in_region = copy.deepcopy(last_pf_in_region)
                    for jj, trained_pg_model in enumerate(trained_pg_models):
                            
                            new_data = np.array(weights).reshape(1, -1)
                            ...
                            t = i-T+1
                            beta_t = 0.5 * np.sqrt(np.log(2*t) / t)
                            pred = mean+beta_t*std
                            temp_last_pf_in_region[ii][jj]+=float(pred)

                    hv = compute_hypervolume(find_pareto_front(list(temp_last_pf_in_region.values())))
                ...
                for weight_idx,idx in zip(sorted_indices,range(weights_per_policy)):
                    next_weight = weights_to_eval[policy_i][weight_idx]
                    weights_to_train[policy_i][idx] = next_weight
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eidx', type=int, default=2, help='The random seed for reproducibility')
    ...
    if MODE  == 0:
        ...
        directory = f'./results/{env_name}/dwc4/'
    ...
    if MODE ==0:
        main(config,env_name, env, env_evaluate, all_weights, eval_weights, weight_dim, MODE,debug=False)
    ...
    if MODE ==1:
        ...
        def evenly_sample_list(input_list, n):
            # Ensure that n is not greater than the list size
            if n > len(input_list):
                raise ValueError("n cannot be greater than the length of the list")
            
            # Calculate the step size to use for evenly sampling
            step = len(input_list) // n
            
            # Sample the list
            sampled_list = [input_list[i] for i in range(0, len(input_list), step)]
            
            # If the list size is less than n due to division truncation, sample the last element too
            if len(sampled_list) < n:
                sampled_list.append(input_list[-1])
            
            # Return exactly n elements
            return sampled_list[:n]
        
        for env_name in ['HalfCheetah-v2','Ant-v2','Hopper-v2', 'Hopper-v3','Walker2d-v2']:

        
            agent_list = main(config,env_name, env, env_evaluate, all_weights, eval_weights, weight_dim, MODE,debug=False)
            for agent in agent_list:
                torch.save(agent.actor_critic.actor.state_dict(), f'./model/{env_name}/dwc4_model/{agent.id}_dwc_run123456_actor_model.pt')  
                agent.actor_critic.actor.load_state_dict(torch.load(f'./model/{env_name}/dwc4_model/{agent.id}_dwc_run123456_actor_model.pt'))
                agent.actor_critic.actor.eval()
                agent.actor_critic.critic.load_state_dict(torch.load(f'./model/{env_name}/dwc4_model/{agent.id}_dwc_run123456_critic_model.pt'))
                agent.actor_critic.critic.eval()
            

            eval_weights = []

            for _ in range(5000):
                w1 = round(random.random(),3)
                w2 = round(1-w1,3)
                eval_weights.append([w1,w2])
            ...
            for x in [100,200,300,400,500,1000,1500,2000,2500,3000,3500,4000,4500,5000]:
                hvs = []
                sps = []

                sampled_eval_weights = evenly_sample_list(eval_weights,x) 

                weights_to_eval = distibute_weight(sampled_eval_weights,policy_num)

                _dict_=  {}

                for agent,weights in zip(agent_list,list(weights_to_eval.values())):
                
                    k=0

                    for _w in weights:
                        k +=1
                        _dict_= evaluate_policy_PF(_dict_, k, agent, _w, env, times=10)
                ...
                with open(f'./{env_name}_hv.json','w') as file:
                    json.dump(hvs,file)
                with open(f'./{env_name}_sp.json','w') as file:
                    json.dump(sps,file)
